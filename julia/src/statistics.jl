...
function compute_aggregates!(model)
    ...
    model.L1 = sum(model[fid].L1 for fid in model.firm1_ids if haskey(model.agents, fid))
    model.Q1 = sum(model[fid].Q1e for fid in model.firm1_ids if haskey(model.agents, fid))
    model.S1 = sum(model[fid].S1 for fid in model.firm1_ids if haskey(model.agents, fid))
    model.D1 = sum(model[fid].D1 for fid in model.firm1_ids if haskey(model.agents, fid))
    
    # Average productivity sector 1
    if !isempty(model.firm1_ids)
        model.A1 = mean(model[fid].A for fid in model.firm1_ids if haskey(model.agents, fid))
    end
    
    # Sector 2 aggregates
    model.F2 = length(model.firm2_ids)
    model.L2 = sum(model[fid].L2 for fid in model.firm2_ids if haskey(model.agents, fid))
    model.Q2 = sum(model[fid].Q2e for fid in model.firm2_ids if haskey(model.agents, fid))
    model.S2 = sum(model[fid].S2 for fid in model.firm2_ids if haskey(model.agents, fid))
    model.D2 = sum(model[fid].D2 for fid in model.firm2_ids if haskey(model.agents, fid))
    
    # Average productivity sector 2
    if !isempty(model.firm2_ids)
        total_k = sum(model[fid].K for fid in model.firm2_ids if haskey(model.agents, fid))
        if total_k > 0
    ...
    model.Div = sum(max(0.0, (model[fid].NW1 * params.d1)) 
                   for fid in model.firm1_ids if haskey(model.agents, fid))
